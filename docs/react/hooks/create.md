为了更好理解`Hooks`原理，这一节遵循`React`的运行流程，实现一个不到100行代码的极简`useState Hook`。建议对照着代码来看本节内容。
...
调用`updateNum`实际调用的是`dispatchAction.bind(null, hook.queue)`，先来了解下这个函数：
...
环状链表操作不太容易理解，这里详细讲解下。

当产生第一个`update`（叫他`u0`），此时`queue.pending === null`。
...
当产生第二个`update`（叫他`u1`），`update.next = queue.pending.next;`，此时`queue.pending.next === u0`，
即`u1.next = u0`。
...
这样做的好处是，当要遍历`update`时，`queue.pending.next`指向第一个插入的`update`。

## 状态如何保存

现在知道，`更新`产生的`update`对象会保存在`queue`中。
...
接下来关注`fiber.memoizedState`中保存的`Hook`的数据结构。
...
在上文`dispatchAction`末尾通过`schedule`方法模拟`React`调度更新流程。
...
用`isMount`变量指代是`mount`还是`update`。
...
在组件`render`时，每当遇到下一个`useState`，移动`workInProgressHook`的指针。
...
至此，完成了一个不到100行代码的`Hooks`。重要的是，他与`React`的运行逻辑相同。
...
[多个state demo](https://code.h5jun.com/woniq/1/edit?js,console)

## 与React的区别

用尽可能少的代码模拟了`Hooks`的运行，但是相比`React Hooks`，他还有很多不足。以下是他与`React Hooks`的区别：
