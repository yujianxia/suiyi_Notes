...
在[双缓存机制一节](/react/process/doubleBuffer.md#什么是-双缓存)介绍过，`workInProgress Fiber树`在`commit阶段`完成渲染后会变为`current Fiber树`。这行代码的作用就是切换`fiberRootNode`指向的`current Fiber树`。
