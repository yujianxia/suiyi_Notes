...
上一节介绍了单一节点的`Diff`，现在考虑有一个`FunctionComponent`：
...
这一节来看看，如何处理同级多个节点的`Diff`。

## 概览

首先归纳下需要处理的情况：

以**之前**代表更新前的`JSX对象`，**之后**代表更新后的`JSX对象`
...
该如何设计算法呢？如果让设计一个`Diff算法`，首先想到的方案是：
...
warning 注意
在做数组相关的算法题时，经常使用**双指针**从数组头和尾同时遍历以提高效率，但是这里却不行。
...
这是`Diff算法`最精髓也是最难懂的部分。接下来会重点讲解。
...
既然的目标是寻找移动的节点，那么需要明确：节点是否移动是以什么为参照物？

的参照物是：最后一个可复用的节点在`oldFiber`中的位置索引（用变量`lastPlacedIndex`表示）。
...
单纯文字表达比较晦涩，这里提供两个Demo，你可以对照着理解。

## Demo1

在Demo中简化下书写，每个字母代表一个节点，字母的值代表节点的`key`
```jsx
...
可以看到，以为从 `abcd` 变为 `dabc`，只需要将`d`移动到前面。
