当有了前面知识的铺垫，就很容易理解`this.setState`的工作流程。
...
在`enqueueSetState`方法中就是熟悉的从`创建update`到`调度update`的流程了。
...
在`this.updater`上，除了`enqueueSetState`外，还存在`enqueueForceUpdate`，当调用`this.forceUpdate`时会调用他。
...
至此，学习完了`HostRoot | ClassComponent`所使用的`Update`的更新流程。

在下一章会学习另一种数据结构的`Update` —— 用于`Hooks`的`Update`。