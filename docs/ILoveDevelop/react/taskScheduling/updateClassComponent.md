...
很熟悉对不对，这几个方法就对应平时调用的`forceUpdate`, `replaceState`, `setState`
...
这里给 `instance` 的`_reactInternalFiber`挂载上`workInProgress`，所以可以通过`this._reactInternalFiber`获取该实例对应的`Fiber`对象
