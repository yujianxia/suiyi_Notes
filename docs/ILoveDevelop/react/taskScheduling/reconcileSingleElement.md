...
第一个`while`循环的目的明显就是找到老的`children`和新的`children`中第一个**key和节点类型**相同的节点，直接复用这个节点，然后删除老的`children`中其他的（无法保证新的`children`是单个节点的时候老的`children`也是单个的，所以要用遍历。）

**注意`key`为`null`也认为是相等，因为单个节点没有`key`也是正常的**
