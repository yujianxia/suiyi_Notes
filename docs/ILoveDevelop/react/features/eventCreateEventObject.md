...
在触发事件的过程中，会调用每个插件的`extractEvents`方法来创建对应的事件，这里就看看事件创建的过程。这里拿最常用的事件之一`onChange`来举例，主要看的是`ChangeEventPlugin.js`
...
`isTextInputElement`判断`input`标签的`type`是否合理，如果是`textarea`则直接返回`true`
