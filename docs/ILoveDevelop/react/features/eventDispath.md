...
之前已经讲了事件是绑定在`container`上的，那么具体事件触发的时候是如何派发到具体的监听者上的呢？这里就往下看，假设绑定的是`dispatchInteractiveEvent`方法，那么来看看他的实现过程：
...
这里简单来说就是调用了`dispatchEvent(topLevelType, nativeEvent)`，`topLevelType`就是`onClick`这类得 React 中得`props`名字，`nativeEvent`就是 DOM 事件对象。那么来看`dispatchEvent`
...
这里主要创建了`bookKeeping`对象，包含了**事件名称，原始事件对象，以及最近的`Fiber`对象**，然后调用`batchedUpdates`，来看看这个方法：
...
`simulated`传入的是`false`，所以调用的是`executeDispatchesAndReleaseTopLevel`
...
`_dispatchListeners`和`_dispatchInstances`是在生成事件对象的时候赋值的，在`executeDispatchesInOrder`中就按照顺序执行了所有监听的事件了。