...
4. 把这部分字符串转为[Blob](https://developer.mozilla.org/zh-CN/docs/Web/API/Blob)对象。

```js
const blob = new Blob([code], { type: 'application/javascript'});
```

5. 再把这个Blob对象转为[DOMString](https://developer.mozilla.org/zh-CN/docs/Web/API/DOMString)。

```js
const worker_script = URL.createObjectURL(blob);
```

6. 这样，就可以在别的文件通过import来访问这个文件对象了：
