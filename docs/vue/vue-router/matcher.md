...
`createMatcher` 接收 2 个参数，一个是 `router`，它是 `new VueRouter` 返回的实例，一个是 `routes`，它是用户定义的路由配置，来看一下之前举的例子中的配置：
...
那么到此，`matcher` 相关的主流程的分析就结束了，了解了 `Location`、`Route`、`RouteRecord` 等概念。并通过 `matcher` 的 `match` 方法，会找到匹配的路径 `Route`，这个对 `Route` 的切换，组件的渲染都有非常重要的指导意义。下一节会回到 `transitionTo` 方法，看一看路径的切换都做了哪些事情。