...
可以使用 2 种方法查看 `Object.assign` 是否可枚举，使用 `Object.getOwnPropertyDescriptor` 或者 `Object.propertyIsEnumerable` 都可以，其中`propertyIsEnumerable(..)` 会检查给定的属性名是否直接存在于对象中（而不是在原型链上）并且满足 `enumerable: true`。具体用法如下：
...
介绍这么多是因为直接在 `Object` 上挂载属性 `a` 之后是可枚举的，来看如下代码。
...
所以在模拟实现 `Object.assign` 时需要使用严格模式。
...
这边使用了 `in` 操作符和 `hasOwnProperty` 方法，区别如下（不知道的JS上卷 P119）：
...
解决方法也很简单，使用在【进阶3-3期】中介绍的 call 就可以了，使用如下。
